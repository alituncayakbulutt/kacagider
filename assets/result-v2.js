(function(){
  ...
  function addStyles(){
    ...
    style.textContent=''
      ...
      +'.kg-result-v2-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px}.kg-result-v2-btn{min-height:41px;border-radius:10px;border:1px solid #d9e0e8;background:#fff;color:#253047;font:inherit;font-size:11.5px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px}.kg-result-v2-btn:hover{border-color:#8ed5aa;background:#f7fff9}.kg-result-v2-btn.primary{background:#0b1628;color:#fff;border-color:#0b1628}.kg-result-v2-btn.primary:hover{background:#087a37;border-color:#087a37}'
  ...
  function setStatus(text){
    ...
    if(el){
      el.textContent=text||'';
      if(text) setTimeout(function(){if(el.textContent===text) el.textContent='';},2800);
    }
  }
  ...
  function saveResult(){
    ...
    try{
      ...
      setStatus('Sonuç bu cihazda kaydedildi.');
  ...
  async function shareResult(){
    ...
    try{
      if(navigator.share){
        ...
        await navigator.clipboard.writeText(text+' https://kacagider.com.tr/');
  ...
  function boot(){
    ...
    ids.forEach(function(id){
      ...
      if(el){ new MutationObserver(render).observe(el,{childList:true,characterData:true,subtree:true}); }
    });
    var trust=document.querySelector('#viewHome .trust p');
