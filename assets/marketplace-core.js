...
function valuationCard(c){
  if(typeof window.KGMarketplaceValuationCardHtml==="function")return window.KGMarketplaceValuationCardHtml(c);
  return '<div class="kg-mp-device"><h3>'+esc([c.brand,c.model,c.storage].filter(Boolean).join(" "))+'</h3><p>KaçaGider piyasa değeri: <strong>'+money(c.marketValue)+'</strong></p></div>';
}
function immutableDetails(c){
  var details=Array.isArray(c.details)?c.details.slice():[];
  if(Number.isFinite(Number(c.score))){details=details.filter(function(x){return !(x&&x.label==='KaçaGider Cihaz Skoru');});details.push({label:'KaçaGider Cihaz Skoru',value:Math.round(Number(c.score))+'/100'+(c.scoreLabel?' · '+c.scoreLabel:'')});}
  return details;
}
