...
function installStyle(){
  ...
  s.textContent=`
    ...
    .kg-account-tabs{display:grid;grid-template-columns:1fr 1fr;background:#f2f4f7;border-radius:11px;padding:4px;margin:0 0 14px}
    .kg-account-tab{border:0;background:transparent;border-radius:8px;padding:10px;font-weight:850;color:#667085;cursor:pointer}
    .kg-account-tab.active{background:#fff;color:#111827;box-shadow:0 1px 3px rgba(15,23,42,.08)}
    ...
    .kg-account-field small{font-size:11px;color:#667085}
    ...
    @media(max-width:640px){
      ...
      .kg-account-tabs{margin-bottom:12px;border-radius:12px}.kg-account-tab{min-height:44px;padding:9px 8px;font-size:13px}
      .kg-account-divider{margin:12px 0;font-size:11px;gap:9px}
      .kg-account-field{gap:7px;margin-bottom:11px}.kg-account-field label{font-size:13px}
      ...
      .kg-mp-overlay:has(#kgMpAuth){align-items:flex-start!important;justify-content:center!important;overflow-y:auto!important;padding:max(10px,env(safe-area-inset-top)) 10px max(10px,env(safe-area-inset-bottom))!important;box-sizing:border-box!important;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}
      .kg-mp-modal:has(#kgMpAuth){width:100%!important;max-width:none!important;margin:auto 0!important;max-height:calc(100dvh - 20px)!important;border-radius:18px!important;overflow-y:auto!important;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}
      ...
      .kg-mp-modal:has(#kgMpAuth) .kg-mp-body{padding:16px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-tabs{margin-bottom:12px!important;border-radius:12px!important}
      ...
      .kg-mp-modal:has(#kgMpAuth) .kg-mp-field{gap:7px!important;margin-bottom:11px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-field label{font-size:13px!important}
      ...
      .kg-mp-modal:has(#kgMpAuth) .kg-mp-field small{font-size:11px!important;line-height:1.35!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-actions{display:block!important;margin-top:12px!important}
    ...
    @media(max-width:380px){.kg-account-login-overlay{padding-left:8px;padding-right:8px}.kg-account-login-card{padding:18px 14px 16px;border-radius:16px}.kg-account-login-card h2{font-size:21px}.kg-account-login-card p{font-size:12px}.kg-account-google,.kg-account-submit,.kg-account-field input{min-height:48px}.kg-mp-overlay:has(#kgMpAuth){padding-left:8px!important;padding-right:8px!important}.kg-mp-modal:has(#kgMpAuth){border-radius:16px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-head{padding:16px 14px 12px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-body{padding:14px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-head h2{font-size:20px!important}}
    @media(max-width:640px) and (max-height:650px){.kg-account-login-card{margin:4px 0;padding-top:16px;padding-bottom:14px}.kg-account-login-card p{margin-bottom:10px}.kg-account-divider{margin:9px 0}.kg-account-field{margin-bottom:8px}.kg-account-google,.kg-account-submit,.kg-account-field input{min-height:46px}.kg-account-forgot{min-height:32px}.kg-mp-modal:has(#kgMpAuth){margin:4px 0!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-head{padding-top:14px!important;padding-bottom:10px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-body{padding-top:12px!important;padding-bottom:12px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-note{margin-bottom:9px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-field{margin-bottom:8px!important}.kg-mp-modal:has(#kgMpAuth) .kg-mp-field input,.kg-mp-modal:has(#kgMpAuth) .kg-mp-btn.primary{min-height:46px!important}}
  `;
...
function openLogin(api,mode){
  installStyle();
  var register=mode==="register";
  var old=document.getElementById("kgAccountLoginOverlay");if(old)old.remove();
  var overlay=document.createElement("div");overlay.id="kgAccountLoginOverlay";overlay.className="kg-account-login-overlay";
  overlay.innerHTML='<section class="kg-account-login-card" role="dialog" aria-modal="true" aria-labelledby="kgAccountLoginTitle"><button type="button" class="kg-account-login-close" aria-label="Kapat">×</button><h2 id="kgAccountLoginTitle">'+(register?'Üye Ol':'Giriş Yap')+'</h2><p>'+(register?'KaçaGider hesabını ücretsiz oluştur.':'KaçaGider hesabına Google veya e-posta ile giriş yap.')+'</p><div class="kg-account-tabs"><button type="button" class="kg-account-tab '+(!register?'active':'')+'" data-account-mode="login">Giriş Yap</button><button type="button" class="kg-account-tab '+(register?'active':'')+'" data-account-mode="register">Üye Ol</button></div><div id="kgAccountLoginNote" class="kg-account-note"></div><button type="button" class="kg-account-google" id="kgAccountGoogle"><b>G</b><span>Google ile devam et</span></button><div class="kg-account-divider">veya e-posta ile</div><form id="kgAccountLoginForm">'+(register?'<div class="kg-account-field"><label>Ad Soyad</label><input id="kgAccountName" type="text" autocomplete="name" required></div>':'')+'<div class="kg-account-field"><label>E-posta</label><input id="kgAccountEmail" type="email" inputmode="email" autocomplete="email" autocapitalize="none" spellcheck="false" enterkeyhint="next" required></div><div class="kg-account-field"><label>Şifre</label><input id="kgAccountPassword" type="password" minlength="8" autocomplete="'+(register?'new-password':'current-password')+'" required><small>En az 8 karakter.</small></div>'+(register?'<div class="kg-account-field"><label>Şifre Tekrarı</label><input id="kgAccountPasswordConfirm" type="password" minlength="8" autocomplete="new-password" required></div>':'')+'<button type="submit" class="kg-account-submit" id="kgAccountSubmit">'+(register?'Ücretsiz Üye Ol':'Giriş Yap')+'</button></form>'+(!register?'<button type="button" class="kg-account-forgot" id="kgAccountForgot">Şifremi unuttum</button>':'')+'</section>';
  ...
  Array.from(overlay.querySelectorAll("[data-account-mode]")).forEach(function(b){b.onclick=function(){openLogin(api,b.dataset.accountMode);};});
  var google=document.getElementById("kgAccountGoogle");google.onclick=async function(){google.disabled=true;try{var result=await api.signInWithGoogle();if(result&&result.error)throw result.error;}catch(error){showNote(error.message||"Google ile giriş başlatılamadı.",true);google.disabled=false;}};
  document.getElementById("kgAccountLoginForm").onsubmit=async function(e){
    e.preventDefault();var submit=document.getElementById("kgAccountSubmit");submit.disabled=true;
    try{
      var email=document.getElementById("kgAccountEmail").value.trim();var password=document.getElementById("kgAccountPassword").value;
      if(register){
        var confirm=document.getElementById("kgAccountPasswordConfirm").value;if(password!==confirm){showNote("Şifreler aynı olmalı.",true);submit.disabled=false;return;}
        var result=await api.signUp({fullName:document.getElementById("kgAccountName").value,email:email,password:password});if(result&&result.error)throw result.error;
        if(!(result.data&&result.data.session)){showNote("Üyeliğin oluşturuldu. E-posta adresine gönderdiğimiz doğrulama bağlantısını aç. Doğruladıktan sonra Giriş Yap sekmesinden giriş yapabilirsin.",false);submit.disabled=false;return;}
      }else{
        var result=await api.signIn({email:email,password:password});if(result&&result.error)throw result.error;cachedUser=result&&result.data?result.data.user:null;closeLogin();renderFallback(cachedUser,api);
      }
    }catch(error){showNote(error.message||(register?"Üyelik oluşturulamadı.":"Giriş yapılamadı."),true);submit.disabled=false;}
  };
...
async function boot(){installStyle();var api=await sync();observeHeader();if(!api)return;var client=await api.init();if(client&&client.auth&&typeof client.auth.onAuthStateChange==="function")client.auth.onAuthStateChange(function(_event,session){cachedUser=session&&session.user?session.user:null;renderFallback(cachedUser,api);});setTimeout(sync,350);setTimeout(sync,900);setTimeout(function(){useHeaderOwnedButton();},1400);}
