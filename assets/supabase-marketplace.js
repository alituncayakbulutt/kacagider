...
async function getUser(){const client=await init();const {data,error}=await client.auth.getUser();if(error&&error.name!=="AuthSessionMissingError"&&error.message!=="Auth session missing!")throw error;return data&&data.user?data.user:null;}
async function signUp({fullName,email,password}){const client=await init();return client.auth.signUp({email,password,options:{data:{full_name:fullName||""},emailRedirectTo:"https://www.kacagider.com.tr/"}});}
async function signIn({email,password}){const client=await init();return client.auth.signInWithPassword({email,password});}
async function signOut(){const client=await init();return client.auth.signOut();}

async function publishListing(input){
  const client=await init(),user=await getUser();if(!user)throw new Error("İlan yayınlamak için giriş yapmalısın.");
  const payload={user_id:user.id,category:categoryKey(input.category),brand:String(input.brand||"").trim(),model:String(input.model||"").trim(),storage:String(input.storage||"").trim()||null,color:String(input.color||"").trim()||null,city:String(input.city||"").trim()||null,district:String(input.district||"").trim()||null,description:String(input.description||"").trim()||null,seller_price:Number(input.salePrice||0)||null,market_value:Number(input.marketValue||0)||null,details:Array.isArray(input.details)?input.details:[],seller_name:String(user.user_metadata&&user.user_metadata.full_name||"").trim()||null,contact_phone:String(input.contactPhone||"").trim()||null,status:"draft",published_at:null};
...
window.KGMarketplaceSupabase={ready:init(),init,getUser,signUp,signIn,signOut,publishListing,listPublished,getListing,categoryKey,categoryLabel,config:{url:SUPABASE_URL}};
})();